...
public class MediaPlaybackActivity extends Activity implements MusicUtils.Defs,
	...
	public void onCreate(Bundle icicle) {
		...
		mAlbum.setOnClickListener(new View.OnClickListener() { // /XXX FUCKING
																// lazy

			@Override
			public void onClick(View v) {
				Intent i = new Intent(MediaPlaybackActivity.this,
						LyricsShowActivity.class);
				startActivity(i);
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		if (MusicUtils.getCurrentAudioId() >= 0) {
			...
			if (MusicUtils.AUDIOEFFECT_AGENCY.isAavailable()) {
				Intent i = new Intent(
						MusicUtils.AUDIOEFFECT_AGENCY.ACTION_DISPLAY_AUDIO_EFFECT_CONTROL_PANEL);
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		try {
			...
			case EFFECTS_PANEL: {
				Intent i = new Intent(
						MusicUtils.AUDIOEFFECT_AGENCY.ACTION_DISPLAY_AUDIO_EFFECT_CONTROL_PANEL);
				i.putExtra(MusicUtils.AUDIOEFFECT_AGENCY.EXTRA_AUDIO_SESSION,
	...
	private void refreshSleepTimeRemain() {
		...
		try {
			...
			// Toast.makeText(MediaPlaybackActivity.this, ""+remain,
			// 1500).show();
		} catch (RemoteException ex) {
			remain = -1; // FIXME
		}
		...
		private void initNumberPicker() {
			try {
				...
				if (onChangedListener == null) {
					onChangedListener = Class
							.forName("android.widget.NumberPicker$OnChangedListener");
				}
				Method setOnChangeListener = c.getMethod("setOnChangeListener",
						onChangedListener);
				Object proxy = Proxy.newProxyInstance(getClassLoader(),
						new Class[] { onChangedListener },
						new InvocationHandler() {

							public Object invoke(Object picker, Method m,
									Object[] args) throws Throwable {
								onChanged(args[0], (Integer) args[1],
										(Integer) args[2]);
								return null;
							}
						});
